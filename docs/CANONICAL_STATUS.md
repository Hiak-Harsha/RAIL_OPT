...
| **"Authentic / Real Recorded Train Audio"** | Primary ambient train bed uses a real field-recording asset (`train-ambience-real.mp3` with 152 BPM beatmap sync); other cue sounds (relay clicks, alerts, teleprinter) remain procedurally synthesized. Spoken PA dispatch uses the Web Speech API with automatic synthesis recovery and audio ducking. | **Real Field Recording Ambient Bed & Asset-Driven Spatial Audio** |
